...
alternatives, and the price. One file holds one decision, and we don't rewrite a
file after acceptance, because a decision that turns out wrong is replaced by a
new ADR linked through `superseded-by`.

Name a file `NNNN-kebab-case-title.md`, and number it sequentially without ever
reusing a number. The template is
[`specs/_templates/adr.md`](../../../specs/_templates/adr.md).

An ADR describes the decision and never the history of the document. It reads as
though the decision was always this one, so it carries no "previously we said",
no "reconsidered after", and no record of which option was adopted first. A
rejected option appears in the alternatives table with the reason we rejected
it, and not with its biography, because git history is where the thinking that
moved is kept. Once an ADR is `accepted` we stop editing it at all, and replace
it with a new ADR linked through `superseded-by`.

## Status, and what it commits us to

Each status below says what the household has agreed to and what an author can
still change in the file.
...
| `accepted`, with `amended-by` | Still in force, with one clause changed by a later ADR, which names the clause. We use it when a decision is right except in a detail, so that changing a sentence doesn't cost a reissue of a long document. The amending ADR is the current word on that clause alone. |
| `deprecated` | No longer applies, and nothing replaced it. |

Every live decision in this register is `accepted`, because the household has
agreed all of them and none is speculative. Three are `superseded`: ADR 0003
became 0029 when per-task routing, spend caps and a residency ladder were added;
ADR 0005 became 0030 when identity stopped being a Telegram id; and ADR 0006
became 0032 when email and password joined the sign-in methods, taking a
purchase off the critical path. Four carry an `amended-by`: 0010 on where
settings live (0034), 0011 and 0017 on the clause about the chart living in
migrations, and 0027 on two clauses, Telegram as the source of identity (0030)
and updates arriving by webhook (0033).

References can be retargeted mechanically when an ADR is superseded, so that
active documents point at the live decision, and we never edit the reasoning
inside a frozen ADR. That's a real commitment: the first time implementation
contradicts one of these, we write a new ADR that supersedes it instead of
editing it, because the reasoning behind a reversal is worth more than a tidy
file.

## How to read this set

Forty-six decisions is a lot to meet at once, so they group like this:
...
can change what, and why the household doesn't lose everything when one person
is unavailable.

### What rests on what

The decisions below come first, because the others are written against them.
...
Two components sit outside self-hosting, both deliberately and both in the
capture path: [0029](0029-model-gateway-and-per-task-routing.md) the model
gateway, and [0027](0027-telegram-as-the-conversational-platform.md) Telegram.
Nothing else leaves the household.
...
The table lists every ADR by number, with its title, status and date. A
superseded entry names the ADR that replaced it.
