...
Build the harness before the thing it tests, and prove each capability by
...
Everything runs locally first (spec R14a–R14e): the same `compose.yaml`, a local
`.env`, Telegram by long polling, the model gateway stubbed. No domain, no
tunnel, nothing purchased. Deployment is a later, separate act in the
implementation plan's own sequence, not a step of this slice.

The slice is deliberately paranoid about two things, because both are silent
failures: a backup nobody has restored, and a job nobody notices has stopped.
Each is proven by sabotage — a deliberately failed verification, a stopped job —
rather than by a green log line.
...
| One `compose.yaml` per environment | Simple to read | Two files drift, and portability (ADR 0002) stops being checkable | rejected |
...
| `pg_dump` inside the app container | One fewer service | Backups then depend on the app being healthy, which is the case they must survive | rejected |

## Affected areas

Everything is new. What this slice creates, and what later slices only extend:
...
- **The Taskfile is the interface.** A person and continuous integration invoke
  the same verbs; nothing is documented as a raw command (ADR 0034).
- **`.env.example` is the deployment's inventory.** A required variable missing at
  startup fails the component loudly, and every variable appears here by name
  with no value.
- **The secret inventory** (spec R9a) is a named list, including the n8n
  encryption key — the one whose loss makes a clean database restore useless.
- **The audit skeleton lands now**, not with the ledger: the table shape and the
  trigger function exist so that spec 0003's tables get auditing by attaching to
  something already tested.
- **Telegram's normalisation boundary** (ADR 0027) is defined here even though
  only the health check uses it: sender, text, attachments, language, and the
  delivery mode behind one variable (ADR 0033).

## Migration and compatibility

Nothing exists yet, so there is nothing to migrate — with one exception that
matters: **the migration mechanism itself is proven before there is anything
valuable in it.** Applying twice changes nothing; a failed migration rolls back
and is not recorded as applied.

Reversibility: the whole slice is `task down` plus deleting volumes. The only
irreversible act is registering the local bot, which is free and separate from
the household's bot (ADR 0033).

## Verification strategy

Database work is pgTAP against a throwaway PostgreSQL; everything else is proven
...
| A3a inventory has two custodians | Checklist, recorded manually — the vault is not machine-readable |
...
|---|---|---|
| The offsite destination is not chosen (Q3) | high | Build against a local restic repository; the offsite target is one variable |
...
| Heartbeat windows produce false alarms | medium | Start generous, tighten once real timings are known (`budgets.md`) |
| The audit trigger shape is wrong for the real schema | medium | It is generic by design (table, row, before, after, actor); spec 0003 attaches to it and will find out early |
...
None. This slice implements decisions already accepted: 0002, 0004, 0008, 0009,
0010, 0015, 0017, 0018, 0019, 0020, 0024, 0033, 0034, 0038 (the conversation
table's shape), 0039 (the tool directory) and 0040 (workflow conventions).
