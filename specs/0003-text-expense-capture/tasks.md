...
- [x] **T1.** The ledger schema: `account`, `account_term`, `transaction`,
      ...
      `original_currency`, `account` gains `reviewed`, `household_setting`
      is new. `ledger_probe` (spec 0002 T4's scaffold) is dropped in this
      same migration.
  - Depends on: —
...
- [x] **T2.** The invariant: postings sum to zero per transaction, a
      deferred constraint trigger, attacked directly before any workflow
      exists. Audit triggers on every new table.
  - Depends on: T1
  ...
  - Done when: A6 (an unbalanced multi-row write is rejected at commit,
      ...
      `test-ledger-invariant.sh`. A5 needs `v_period_spend` and is proven
      in T5, once that view exists.
...
- [x] **T4.** `conversation`, `capture` (one row per message) and
      ...
      order the exchange; `direction` distinguishes inbound from the
      agent's own outbound questions.
  - Depends on: T3
  ...
  - Done when: pgTAP proves a multi-row `capture` exchange orders by
      ...
      after a fresh connection (the shape ADR 0038 requires, proven
      structurally here — A39 proves it end to end once workflows exist).
...
- [x] **T5.** The views this slice needs, every one
  ...
  - Depends on: T4
  - Requirements: R5, R11b, R22, ADR 0040's "nothing computes a figure
      outside a view"
  - Done when: pgTAP impersonation proves each view respects RLS (reading
      ...
      that leaves `v_period_spend` unchanged for the month; spending the
      cash is what moves it) passes against the real view.
...
- [x] **T7.** Prompts and tools as files: `prompts/capture-text/` (system
      ...
      the tool declarations in `tools/` this slice needs — `record`,
      `correct`, `confirm`, `classify`, `delete`, `open_account`,
      `amend_account`, `merge_category`, `undo` — each naming its
...
- [x] **T8.** The capture-text workflow: the entry point
      (`telegram-normalize-and-dispatch`) gains one step — load the
      member's open conversation — then dispatches to a new
      `capture-text` workflow. Merchant recognition, category
  ...
  - Done when: A7, A8, A9, A10, A11, A12, A23, A23a, A24, A26, A34, A37 all
      ...
      gateway; A21 (an unlinked Telegram id produces no transaction —
      spec 0002 T12's own gate, extended here with that assertion) and
...
- [x] **T9.** Unparsed captures and the multi-question agent: no
      ...
      in order, resuming on the next message.
  - Depends on: T8
  - Requirements: R17
...
- [x] **T10.** The setup conversation: the agent asks for accounts,
      ...
      and default currency — one question at a time, resumable, useful
      after the first account. Structural changes (open, rename,
      deactivate, re-term an account; rename, re-parent, merge a
      category) restated before applying, each minted as the asking
      admin's own role (ADR 0042), never the agent's.
  - Depends on: T9
  - Requirements: R0, R0a, R0b, R0c, R0d, R0e, R0f, R29
  - Done when: A1, A2, A3, A4, A38, A39 all pass — A39 specifically by
...
- [x] **T11.** Corrections and the request path: an admin corrects amount,
  ...
  - Done when: A13, A14, A15, A16 pass in `test-corrections.sh`, each
      minted as the acting member's own role (ADR 0042) so A17's refusal
      is the database's, not the workflow's.

- [x] **T12.** `auto-confirm-quiet`, the one scheduled workflow: a known
      merchant, the merchant's own category, below the configured
      threshold, past the configured quiet period — confirmed with its
      route recorded. It runs from the scheduler container like every
      other job.
  - Depends on: T11
...
- [x] **T13.** The conveniences: inline buttons with a typed equivalent
      ...
      on a transaction, "как обычно"/"same again", the recent-captures
      list, "what still needs setting up", and the agent's own memory
      (ADR 0044) — the `remember` tool, and reading relevant memory when
      composing a reply.
  - Depends on: T12
...
- [x] **T14.** Chat administration of the unconfirmed queue, and the
      declared-tool negative tests across the whole surface built by T7–T13.
  - Depends on: T13
  - Requirements: R20, R11cc
  - Done when: A20's admin-facing counterpart — A43 (list and confirm the
      queue in chat) — passes, and A42 passes for every tool declared in
      T7: malformed input writes nothing, and a member without the
      permission fails at the database.
...
- [x] **T15.** The golden set's first run, and documentation:
  ...
  - Done when: the golden set runs against a real model (not the stub,
      not part of `task test`) and its scores are recorded in this
