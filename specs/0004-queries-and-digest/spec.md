...
After spec 0003 the household puts expenses into the books and gets nothing
back, which is how ledgers get abandoned: a month of feeding a system that never
answers, and then nobody bothers.

Two separate things are missing. Nobody can *ask* anything, because "how much on
groceries this month" has no answer short of SQL, and nobody is *told* anything,
so only a member who deliberately goes looking ever sees the books - which after
week two is nobody.

## Why

After this slice, the books answer questions in the chat where they were filled
in, and a digest arrives without anyone asking for it. The digest matters more,
because it is what makes the household notice the books exist at all.

We will know it worked when a question in plain words gets a number the admins
trust, and the household reads the weekly digest instead of muting it.
...
These requirements cover what a member can ask, where the figure in the answer
comes from, and what happens when no tool answers the question.

- **R1.** A member must be able to ask about the books in free text, in Russian
  or English, and receive a figure with the period it covers.
- **R2.** The system must answer at least these questions, asked in free text.
  Spending: the total for a period; by category; at a merchant; by a member;
  which categories took the most in a period, ranked; and any of those compared
  with the previous period. The state of the accounts: what a named account
  holds; what the household holds in total; what it owes in total and per
  liability; its net position; and how much headroom is left against an
  overdraft or credit limit. Movement: what changed on an account over a period,
  and what the largest expenses in a period were.
- **R2a.** Every later slice that adds a view must extend the answerable set. A
  slice that produces a figure for the app without making it askable in chat is
  incomplete, because the product is chat-first and a number reachable only on a
  screen is a regression. Each such slice adds its own read tool and its entry
  in the documented list, since ADR 0046 holds that a capability with no tool is
  one the household cannot ask for.
- **R3.** Every answer must state the period it covers and the unconfirmed share
  of the figures in it (ADR 0023).
- **R4.** Every figure must come from a SQL view, and never from arithmetic
  performed in a workflow or by the model.
- **R4a.** Every figure in a reply must be traceable to the tool result it came
  ...
  in the reply (ADR 0046). This replaces the guarantee a templated answer used
  to give, now that the agent writes its own.
- **R5.** The agent must answer by calling declared tools and reading their
  ...
  (ADR 0046).
- **R6.** The system must refuse a question no declared tool answers, plainly,
  and say what can be asked instead. It must never guess a figure, and never
  answer from the nearest tool that does not actually answer the question.
- **R7.** An answer must be a short message rather than a table dump, must be in
  the member's own language, and must follow
  `docs/standards/agent-persona.md`. The agent writes it (ADR 0046) instead of
  selecting it from a catalogue, so we test the persona as a property rather
  than rely on a string having been written in advance.
- **R8.** A member must be able to ask about the whole household's books
  (ADR 0016).

### Telling

These requirements cover the digests that arrive without being asked for, and
the further reading paths - search, provenance and export - that a member
reaches from an answer.
...
- **R13.** A digest must be generated from the same views that answer questions,
  so that a digest and an answer can never disagree.
- **R14.** Each digest must push a heartbeat on success, so that a digest that
  stops being sent raises an alert (ADR 0020).
- **R15.** A member must be able to turn their own digests off, and on.
- **R16.** A digest for a period with no data must say so in one line rather
  than be suppressed silently.
...
- **R19.** A member must be able to search the books in free text, by a
  merchant, a note or a fragment of a capture, and receive matching transactions
  with their dates and amounts.
- **R20.** A member must be able to ask why a transaction is categorised as it
  is, and be told whether the category came from the merchant's default, from an
  explicit choice, or from the model, with the prompt version in the last case
  (ADRs 0008, 0025).
- **R21.** An admin must be able to export a period as CSV, covering
  transactions, postings and balances, delivered in chat or downloaded from the
  app. The books belong to the household and must be removable from it at any
  time.
- **R22.** Where a question offers a closed choice, such as which period or
  which category, the bot must offer it as buttons with a typed equivalent
  (ADR 0035).
...
- Charts and trends over time (spec 0006), because a chart belongs on a screen.
- Forecasts, budgets and "can we afford" questions (spec 0009), because there is
  nothing to forecast from yet.
- Project totals (spec 0010).
- Statement-based reconciliation status beyond the unconfirmed share (spec
  0007).
- Analysis beyond reading the books back. The declared tool set is the contract:
  the agent can combine tools to answer a question nobody anticipated, but it
...
- [x] **A14.** Given any question, when the reply and the tool results that
      ...
      tool result and no figure appears that does not, because the agent
      transcribes what the database returned and computes nothing (R4a,
      ADR 0046).
...
- [x] **A16.** Given the schedules, when Monday 09:00 household time passes, then
      ...
      been sent, without waiting for the month to be reconciled, and stating its
...
The table pairs each situation a question or a digest can land in with what the
system does about it. The states are named from
...
| A digest period where every record is unconfirmed | Sent, with the share stated as 100%, because the figure is still the best available |
...
- **What queue or obligation it creates:** none. A digest never becomes an
  obligation, because R15 lets a member turn it off.
- **What it interrupts, and how often:** twice a week at most, per member: one
  weekly digest, one monthly. That is inside the interruption budget in
...
- **If nobody touches it for a month:** digests keep arriving and can be ignored,
  and a digest that stops arriving raises an alert. Nothing accumulates.
...
- **Cost:** a handful of model calls per question, one per round of the agent's
  loop, bounded by the round cap (ADR 0046). A digest costs nothing at all,
  because it is SQL and a template with no model involved.
- **Data minimisation:** the agent receives the question, the tool list, and the
  rows the tools it called returned, which are the figures it is about to report
  and nothing else. Nobody hands it the ledger to search through, and a tool
