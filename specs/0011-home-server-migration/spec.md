...
We promised portability in the first decision we made: everything that runs on
rented hosting must move to the household's own hardware without being rewritten
(ADR 0002). Nobody has tested that promise, so it is still a belief, and beliefs
about infrastructure usually turn out wrong in the places nobody looked at.

Three other promises sit in the same position. The rebuild procedure claims that
an empty host plus this repository plus the secrets plus a backup produce the
same system (ADR 0010). The backups claim to be restorable (ADR 0009). And
secret custody claims that either admin can recover alone, from a sealed copy and
the written procedure (ADR 0024), which the admin who did not write that
procedure has never attempted.

This slice proves all four by doing them.

## Why

After this slice the household's financial system runs on hardware it owns, and
the claims above are facts instead of intentions. The rehearsal is as much the
deliverable as the migration, because it is a restore performed by the second
admin, unaided.

We will know this worked when the system runs at home, nothing was lost, no
workflow was changed, and the admin who did not build it recovered the database
alone.
...
- **R1.** The system must run on the home server from the same
  `docker-compose.yml` used on the rented host, with only environment variables
  and DNS differing (ADR 0002).
- **R2.** Nobody may modify a workflow, migration, prompt or application file to
  make the migration work. If one must be modified, the portability claim was
  false, and we record the change as a finding before making it.
- **R3.** The public hostname must not change, because passkeys are bound to it
  (ADR 0032). If it must change, every passkey is re-enrolled as part of this
  slice.
- **R4.** The public edge must stay reachable with no inbound port opened on the
  household network.
- **R5.** All data must be carried over: the database, the file volume, and the
  state of every self-hosted component.
- **R6.** Members must not have to do anything: no re-linking the bot, no new
...
- **R8.** Backups must resume to EU offsite storage from the new location, and
  the first verification after the move must pass (ADR 0009).
- **R9.** Nobody may decommission the rented host until the home server has run
...
- **R10.** The second admin must perform a full restore of the database and the
  ...
  and their own copy of the secrets (ADR 0024). Any step they cannot complete is
  a defect in the procedure, not in them.
- **R11.** The rebuild claim must be demonstrated: an empty host, this
  repository, the secrets and the latest backup must produce a working system
  (ADR 0010).
- **R12.** Data integrity after the move must be verified rather than assumed:
  row counts, account balances, and the file volume's hashes against the rows
  that reference them.
- **R13.** The residency position must be re-stated after the move: what is now
  in the EU, and which of ADR 0028's gaps have closed, because the move makes at
  least one of them cheaper to close.
...
- **R16.** The edge must not change in this slice. We change the host and the
  edge separately, so that a failure can only have one cause.

## Scope

**In scope:** provisioning the home server, the edge arrangement for the home
phase, carrying the data, cutting over, verifying integrity, the second admin's
unaided restore rehearsal, the rebuild demonstration, monitoring and backups from
the new location, the rollback plan, and updating the procedure with everything
...
- Any product change. A wanted feature is a different slice, and mixing one in
  would leave us unable to say why a failed migration failed.
...
- [ ] **A3.** Given the cutover, when a member sends a capture from Telegram, then it
      is recorded, which proves the edge, the bot and the workflows without anyone
      re-linking anything.
...
- [ ] **A12.** Given the migration, when the edge configuration before and after is
      compared, then it is unchanged, because the host moved and the edge did not.
...
| The second admin's rehearsal fails | The procedure is wrong. We fix it and repeat the rehearsal, because the rehearsal is the criterion and not a formality |
...
- **Who does more work:** both admins, once, for a few hours. Afterwards the
  household owns its uptime, which is a real ongoing cost, because a power cut
  becomes the household's problem.
...
- **If nobody touches it for a month:** it runs, and the monthly verification is
  the message that says so. What changes from the rented phase is that a hardware
...
- Downtime during cutover lasts minutes, at a time when nobody is shopping, so it
  is short enough not to matter.
- The cutover loses no data: it takes a fresh backup first, and a capture sent
  during the window must survive.
- The household's own hardware sits in the EU by construction, which is the
  easiest residency win in the whole system (ADR 0028).
